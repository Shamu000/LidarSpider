...
class LeggedRobotRayCast(LeggedRobot):
    ...
    #     # 如果启用外部LiDAR，应用高级噪声模型  
    #     if hasattr(self.cfg.raycaster, 'use_omni_lidar') and self.cfg.raycaster.use_omni_lidar:  
    #         if hasattr(self, 'omni_lidar'):  
    #             distances = self.omni_lidar.simulate(origins, self.base_quat, self.ray_caster)  
    #         else:  
    #             # 初始化OmniLiDAR  
    #             from .omni_lidar import OmniLiDAR  
    #             self.omni_lidar = OmniLiDAR(self.cfg.raycaster, self.device)  
    #             distances = self.omni_lidar.simulate(origins, self.base_quat, self.ray_caster)  
