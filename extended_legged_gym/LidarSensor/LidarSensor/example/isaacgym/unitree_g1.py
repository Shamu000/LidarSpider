...
def downsample_spherical_points_vectorized(sphere_points, num_theta_bins=10, num_phi_bins=10): # 球面坐标点云进行二维角度网格划分
    ...
    Args:
        ...
        num_theta_bins: Number of bins for theta range (-3.14, 3.14)水平像素数量
        num_phi_bins: Number of bins for phi range (-0.12, 0.907)垂直像素数量
...
class G1Env:
    def __init__(self, 
        ...
        # extended_legged_gym/legged_gym/legged_gym/envs/base/base_task.py/42
        self.gym = gymapi.acquire_gym() # 获取核心 gym 控制对象，包含所有仿真 API
        self.num_envs = num_envs
        self.num_obstacles = num_obstacles
        self.headless = args.headless
        self.show_viewer = not args.headless # headless为True：没有 GUI
        self.device = "cuda" if torch.cuda.is_available() else "cpu"
        
        self.sensor_cfg = LidarConfig() # Lidar 配置对象
        ...
        self.sensor_cfg.update_frequency # 传感器更新频率
        ...
        # Create ground plane
        self.create_ground()
        self.create_viewer() # 创建可视化窗口
        # Create obstacles
        self.create_env()
        
        self.gym.prepare_sim(self.sim) # 准备整个仿真（编译/上传）
        
        self._init_buffer() # 绑定 Isaac Gym root state，与 GPU 张量同步
        self.create_warp_env() # Warp 格式的 mesh 用于 Lidar 仿真

        self.crete_warp_tensor() # GPU 张量（点云输出/距离输出/姿态）
        
        self.sensor = LidarSensor(self.warp_tensor_dict, None, self.sensor_cfg, 1, self.device)
        self.lidar_tensor, self.sensor_dist_tensor = self.sensor.update() # 雷达扫描， 点， 距离
        ...
        # Initialize target position and orientation for all environments 用于随机移动
        ...
        if self.enable_random_movement:
            ...
            #self.env_origins = torch.zeros(self.num_envs, 3, device=self.device, requires_grad=False) 获取地形尺寸
    ...
    def generate_random_targets(self):
        ...
        # Generate random velocities for all environments at once 最大幅度范围
        ...
        # Reset the timer with variation to prevent all robots from changing velocities at the same time 重置的计时器
    ...
    def move_to_targets(self):
        ...
        if not hasattr(self, 'target_lin_vel') or not hasattr(self, 'target_ang_vel'): # 判断self中是否有这两个属性
            return
        ...
        # Check position constraints and reverse velocities if needed 边界限制
        ...
        if torch.any(x_too_low) or torch.any(x_too_high):
            ...
            new_pos[:, 0] = torch.clamp(new_pos[:, 0], 0, terrain_x_limit) # 限制在范围内
    ...
    def _init_height_points(self):
        ...
        grid_x, grid_y = torch.meshgrid(x, y) # 局部测点二维网格

        self.num_height_points = grid_x.numel()
        points = torch.zeros(self.num_envs, self.num_height_points, 3, device=self.device, requires_grad=False)
        for i in range(self.num_envs): # 随机噪声
